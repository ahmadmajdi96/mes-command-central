...
import {
  findWO, findPO, findProduct, findWorkstation, routings, findUser,
} from "@/lib/oms-data";
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            {wo.status === "pending" && <button className="flex items-center gap-1.5 rounded-lg border border-primary/40 bg-primary/10 px-3 py-1.5 text-xs font-medium text-primary"><Play className="h-3.5 w-3.5" /> Start</button>}
            {wo.status === "in_progress" && <>
              <button className="flex items-center gap-1.5 rounded-lg border border-warning/40 bg-warning/10 px-3 py-1.5 text-xs font-medium text-warning"><Pause className="h-3.5 w-3.5" /> Pause</button>
              <button className="flex items-center gap-1.5 rounded-lg border border-success/40 bg-success/10 px-3 py-1.5 text-xs font-medium text-success"><CheckCircle2 className="h-3.5 w-3.5" /> Complete</button>
            </>}
            <button className="flex items-center gap-1.5 rounded-lg border border-destructive/40 bg-destructive/10 px-3 py-1.5 text-xs text-destructive"><AlertTriangle className="h-3.5 w-3.5" /> Report Issue</button>
          </div>
        }
      />
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Panel className="lg:col-span-2">
          ...
          <div className="mb-2 flex items-baseline justify-between">
            <span className="font-mono text-4xl font-semibold text-glow">{wo.qtyProduced}<span className="text-xl text-muted-foreground"> / {wo.qtyTarget}</span></span>
            <div className="text-right">
              <div className="font-mono text-lg font-semibold">{wo.progress}%</div>
        ...
        <Panel>
          ...
          <div className="space-y-3">
            <div>
              ...
              <input type="number" defaultValue={wo.qtyProduced} className="mt-1 h-9 w-full rounded-lg border border-border/60 bg-card/60 px-3 font-mono text-sm focus:border-primary/50 focus:outline-none" />
            </div>
            <div>
              <label className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Scrap qty</label>
              <input type="number" defaultValue={wo.qtyScrap} className="mt-1 h-9 w-full rounded-lg border border-border/60 bg-card/60 px-3 font-mono text-sm focus:border-primary/50 focus:outline-none" />
            </div>
            <div>
              <label className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Labor min</label>
              <input type="number" defaultValue={wo.laborMin} className="mt-1 h-9 w-full rounded-lg border border-border/60 bg-card/60 px-3 font-mono text-sm focus:border-primary/50 focus:outline-none" />
            </div>
            <button className="w-full rounded-lg bg-gradient-to-r from-primary to-info px-3 py-2 text-xs font-medium text-primary-foreground">Log Progress</button>
          </div>
        </Panel>
      </div>
