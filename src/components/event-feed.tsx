...
import { Bell, X, Play, Pause, CheckCircle2, Circle, MessageSquare, Activity } from "lucide-react";
import { useStore, store, type FeedEventKind } from "@/lib/store";

const iconFor = (kind: FeedEventKind) => {
  switch (kind) {
    case "wo_start": return <Play className="h-3.5 w-3.5 text-success" />;
    case "wo_resume": return <Play className="h-3.5 w-3.5 text-success" />;
    ...
    <>
      ...
      {open && (
        <>
          ...
          <div className="glass-panel absolute right-4 top-14 z-50 flex max-h-[70vh] w-[360px] flex-col rounded-2xl border border-border/60 shadow-xl">
            <div className="flex items-center justify-between border-b border-border/60 p-3">
              <div>
                <h3 className="text-sm font-semibold">Event Feed</h3>
                <p className="text-[11px] text-muted-foreground">Real-time WO & SOP changes</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-2">
              {events.length === 0 ? (
                ...
                <ul className="space-y-1">
                  {events.map((e) => (
                    <li key={e.id} className="flex items-start gap-2 rounded-lg border border-border/40 bg-card/40 p-2">
